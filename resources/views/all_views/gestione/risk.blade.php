...
@section('content_main')
  ...
  <div class="content-wrapper">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Voci RISK</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    ...
    <div class="content">
      <div class="container-fluid">
		...
		@include('all_views.gestione.newrisk')

		<form method='post' action="{{ route('risk') }}" id='frm_risk' name='frm_risk' autocomplete="off">
        ...
        <div class="row">
          <div class="col-md-12">
				...
				<table id='tbl_list_risk' class="display">
					<thead>
						<tr>
							<th>ID</th>
							<th>Descrizione</th>
					...
					<tbody>
						@foreach($risk as $risk_value)
							<tr>
								<td>{{ $risk_value->id }}</td>	
								<td>
								 @if ($risk_value->dele=="1") 
									<font color='red'><del> 
								 @endif
									<span id='id_descr{{$risk_value->id}}' data-descr='{{ $risk_value->voce }}'>
										{{ $risk_value->voce }}
									</span>	
								 @if ($risk_value->dele=="1") 
									 </del></font>
			...
			<div class="row">
			    <div class="col-lg-12">
					<button type="button" class="btn btn-primary" onclick="$('#edit_elem').val('');$('#descr_contr').val('');$('#div_definition').show(150)">
						<i class="fa fa-plus-circle"></i> Nuova Voce RISK
					</button>
					<div class="form-check form-switch mt-3 ml-3">
					  <input class="form-check-input" type="checkbox" id="view_dele" name="view_dele" onchange="$('#frm_risk').submit()" {{ $check }}>
 ...
 @section('content_plugin')
	...
	<script src="{{ URL::asset('/') }}dist/js/risk.js?ver=1.2"></script>
